...
install_reflector_mode() {
    ...
    # Add country filter
    if [ -n "$COUNTRY" ]; then
        local countries=$(echo "$COUNTRY" | tr ',' ' ')
        reflector_cmd="$reflector_cmd --country $countries"
    fi
...
install_official_mode() {
    ...
    local api_url="https://archlinux.org/mirrorlist/?"
    [ -n "$COUNTRY" ] && api_url="${api_url}&country=$(echo "$COUNTRY" | tr ',' '&country=')"
    [ -n "$PROTOCOL" ] && api_url="${api_url}&protocol=$PROTOCOL"
    api_url="${api_url}&use_mirror_status=on"
